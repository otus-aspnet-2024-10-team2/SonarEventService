...
namespace WebApi.Controllers
{
    ...
    public class SonarTaskController: ControllerBase
    {
        ...
        public SonarTaskController(ISearchTaskService service, ILogger<SonarTaskController> logger, IMapper mapper)
        {
            _service = service;
            _logger = logger;
            _mapper = mapper;
        ...
        public async Task<IActionResult> GetAsync(int id, CancellationToken cancellationToken)
        {
            ...
            return Ok(_mapper.Map<SearchTaskDto, SonarTaskModel>(sonarTaskDto));
        }

        [HttpPost]
        public async Task<IActionResult> AddAsync(CreatingSonarTaskModel creatingSonarTaskDto)
        {
            return Ok(await _service.CreateAsync(_mapper.Map<CreatingSonarTaskModel, CreatingSearchTaskDto>(creatingSonarTaskDto)));
        }
        
        [HttpPut("{id}")]
        public async Task<IActionResult> EditAsync(int id, UpdatingSonarTaskModel creatingLessonDto)
        {
            await _service.UpdateAsync(id, _mapper.Map<UpdatingSonarTaskModel, UpdatingSearchTaskDto>(creatingLessonDto));
            return Ok();
        }
